const experienceData = {

  /* ── Work Experience ── */
  experience: [
    {
      title: "10x Growth Flutter Dev Intern",
      org: "NGO Startup",
      period: "Mar 2025 — June 2025",
      location: "Remote",
      description:
        "Designed and implemented a statefull and stateless widget architecture for the NGO's mobile app, enabling dynamic content rendering and seamless user interactions.",
      tags: ["Flutter", "Dart", "Supabase", "GitHub"],
    },
    {
      title: "Django-DRF Backend Engineer Intern",
      org: "Online Travel Agency Startup",
      period: "Mar 2026 — Apr 2026",
      location: "Remote",
      description:
        "Developed a robust backend API's using Django and Django REST Framework to support the startup's travel booking platform. Worked on features such as Channel Management, PMS via Bakun Api.",
      tags: ["Django", "Django REST Framework", "PostgreSQL", "JWT"],
    },
  ],


  /* ── Certifications ── */
  certifications: [
    { title: "AWS Practitioner", issuer: "Udemy", year: "Present" },
    { title: "SQL BootCamp", issuer:"Udemy", year:"2026"},
    { title: "Python for AIML", issuer: "SAP Edunet Foundation", year: "2024" },
    { title: "Advanced Python Course", issuer: "Udemy", year: "2023" },
    
  ],

  /* ── Competitive Achievements ── */
  achievements: [
    {
      rank: "01",
      badge: "🏆",
      title: "1st Place — Algorithm Development",
      org: "TCSC Hackathon 2025",
      period: "2025",
      description:
        "Secured first place in the algorithm development track competing against teams from across Mumbai. Designed an optimised pathfinding solution under time constraints.",
      tags: ["Algorithms", "Python", "Competitive"],
    },
    {
      rank: "02",
      badge: "🎖",
      title: "Smart India Hackathon Finalist",
      org: "Government of India",
      period: "2024",
      description:
        "National finalist — one of the top teams selected from thousands of entries across India. Built a game-based adaptive learning platform evaluated by industry and government panellists.",
      tags: ["National Level", "EdTech", "Team Lead"],
    },
    {
      rank: "03",
      badge: "🥉",
      title: "Top 10 — NMIMS HACKX 3.0",
      org: "NMIMS University",
      period: "2025",
      description:
        "Ranked in the top 10 among over 200 competing teams from premier engineering colleges across Maharashtra.",
      tags: ["Top 10", "200+ Teams", "Maharashtra"],
    },
  ],

  /* ── Activities & Leadership ── */
  activities: [
    {
      icon: "⚙",
      title: "Joint Technical Head",
      org: "Student's Council, UCOE",
      period: "2025 — 2026",
      description:
        "Lead technical strategy and execution for the college student council. Maintained college event website.",
    },
    {
      icon: "⚙",
      title: "Joint PRO Head",
      org: "Student's Council, UCOE",
      period: "2024 — 2025",
      description:
        "Managed public relations and communications for the student council. Coordinated with external stakeholders to promote college events and initiatives.",
    },
    {
      icon: "⚙",
      title: "Joint Documentation Head",
      org: "NSS unit, UCOE",
      period: "2023 — 2024",
      description:
        "Oversaw documentation and reporting for the college's National Service Scheme unit. Ensured accurate record-keeping of activities and events.",
    },
    
    
  ],
};
