...
// ─────────────────────────────────────────────────────────────────────────────
const USE_VERTICAL_NAV = false; // toggle this to switch modes
...
(function () {
  ...
  function initVertical() {
    ...
    sidebar.innerHTML = `
    <a class="vnav-logo" href="#hero">HN<span>.</span></a>
    <div class="vnav-items">
      ${NAV_ITEMS.map(item => `
        <a class="vnav-item" href="${item.href}">
    ...
    <div class="vnav-bottom">
      <button id="vnav-theme" class="vnav-item">
  ...
  `;
    ...
    // 🔴 CRITICAL FIX: reset both dimensions explicitly
    if (progress) {
      progress.style.width = "2px";   // lock width
      progress.style.height = "0%";   // reset height
    }
    ...
    function onScroll() {
      ...
      if (progress) {
        progress.style.width = "2px"; // enforce every frame
